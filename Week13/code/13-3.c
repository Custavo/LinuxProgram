...
static int sig_cnt[NSIG]; // NSIG = 65
...
int main(int argc, char *argv[])
{
	...
	if(argc > 1)
	{
		...
		{
			fprintf(stderr, "setprocmask to block all signal filed(%s)\n", strerror(errno));
			return -2;
		}
		printf("I will sleep %d seconds", sleep_time);
		sleep(sleep_time);
		sigfillset(&empty_mask);
		if(sigprocmask(SIG_SETMASK, &empty_mask, NULL) == -1)
		{
			fprintf(stderr, "setprocmask to release all signal filed(%s)\n", strerror(errno));
			return -3;
		}
	}
